...
#RotaryEmbedding 预先计算并缓存 RoPE 所需的 cos/sin 位置编码，用于在 attention 里给 Q/K 注入相对位置信息。
class RotaryEmbedding(nn.Module):
    ...
    """
    #dim：RoPE 作用的维度，一般是 attention 的 head_dim
    # max_seq_len：最多提前缓存多长的序列位置
    # base：频率基数，默认 10000
...
#对 attention 中的 Q/K 张量施加 Rotary Position Embedding，使其在点积注意力中携带位置信息。
...
#先把输入变成 Q/K/V，多头并行看信息，加上位置感 RoPE，屏蔽 padding，然后算 attention，最后输出融合后的 token。
...
#把所有 Q token 和 NS token 放到一起，做一次轻量级的信息搅拌和增强。
...
#把 user/item 的离散非序列特征 转成 NS token。
