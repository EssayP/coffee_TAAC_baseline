...
"""
"""PCVR Parquet 数据集模块（性能优化版）。

直接读取原始的多列 Parquet 文件，并从
``schema.json`` 中获取特征元信息。

优化点：
- 预先分配 numpy 缓冲区，消除 ``np.zeros`` + ``np.stack`` 带来的额外开销。
- 将多个序列域的 padding 循环融合在一起，直接写入一个 3D 缓冲区。
- 预先计算列名到列索引的查找表，避免每一行都进行字符串查找。
- 使用 ``file_system`` 张量共享策略，绕过在使用大量 DataLoader workers 时
  ``/dev/shm`` 空间耗尽的问题。
"""
...
# numpy.typing 从 numpy >= 1.20 开始可用；在更老版本的 numpy 中，
# 会退回使用一个“空操作”的兼容 shim。
#
# 这样可以保证像 ``npt.NDArray[np.int64]`` 这种前向引用形式的类型注解，
# 在作为普通字符串保留时，仍然能够正常工作，
# 不会在 import 导入阶段直接报错。
try:
...
class FeatureSchema:
    ...
    """为每个特征记录 ``(feature_id, offset, length)``，
    这样下游代码就可以定位 flattened tensor 中属于某个具体 feature id 的那一段。

    对于 int 类型特征：
      - int_value：length = 1
      - int_array：length = 数组长度
      - int_array_and_float_array：int 部分的长度

    对于 dense 类型特征：
      - float_value：length = 1
      - float_array：length = 数组长度
      - int_array_and_float_array：float 部分的长度
    """
    ...
    def __init__(self) -> None:
        ...
        # feature_id：这个特征是谁，比如 fid=48
        # offset：这个特征从拼接 tensor 的第几个位置开始
        # length：这个特征连续占几个位置
        self._fid_to_entry: Dict[int, Tuple[int, int]] = {}

    #往 FeatureSchema 这张“特征布局表”里注册一个新特征。
    ...
    #根据 feature_id，快速查这个特征在拼接 tensor 里的位置。
    ...
    # 把 FeatureSchema 对象变成普通 Python 字典。
    ...
    #反序列化函数,之前把 schema 存成字典了，现在要把它重新变回 FeatureSchema 对象。
    @classmethod
...
#使用基于文件系统的 tensor 共享方式，而不是默认的 /dev/shm 共享内存，
#这样可以避免在开启很多 DataLoader workers 时共享内存耗尽。

torch.multiprocessing.set_sharing_strategy('file_system')

#时间差分桶边界 time_diff = 当前样本时间 - 历史行为时间
...
#时间桶一共有多少种 ID，必须由 BUCKET_BOUNDARIES 自动决定，并且模型里的 time embedding 表大小必须和它完全一致，否则会 embedding 越界报错。
...
class PCVRParquetDataset(IterableDataset):
    ...
    # 它负责从Parquet文件里读取原始训练数据，然后整理成模型可以直接吃的batch。
...
#重点！！选择了什么特征进模型
    ...
    def _flush_buffer(
        ...
        for i in range(0, total_rows, self.batch_size):
            end = min(i + seoklf.batch_size, total_rows)
    ...
    #一批原始 parquet 数据最终变成什么 tensor 喂给模型。
